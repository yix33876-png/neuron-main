...
typedef enum {
    MQTT_AGGREGATE_DISABLED          = 0,
    MQTT_AGGREGATE_RAW_BATCH         = 1,
    MQTT_AGGREGATE_AGG_SINGLE_SENSOR = 2,
    MQTT_AGGREGATE_AGG_MULTI_SENSOR  = 3,
} mqtt_aggregate_mode_e;
...
typedef struct {
    ...
    bool     auth_enable;         // add payload auth fields before publish
    char *   keylink;             // fallback HMAC key
    char *   auth_secret;         // preferred HMAC key, not sent
    mqtt_aggregate_mode_e aggregate_mode;
    uint32_t              aggregate_window_ms;
    uint32_t              sensor_timeout_ms;
    bool                  emit_partial;
    char *                aggregate_topic;
    char *                aggregate_sensor;
    size_t            n_schema_vt;
