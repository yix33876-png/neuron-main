...
{
    mqtt_aggregate_stop_timer(plugin);
...
int mqtt_plugin_config(neu_plugin_t *plugin, const char *setting)
{
    ...
    if (0 != mqtt_aggregate_start_timer(plugin)) {
        plog_error(plugin, "start aggregate timer failed");
        rv = NEU_ERR_EINTERNAL;
        mqtt_config_fini(&plugin->config);
...
int mqtt_plugin_start(neu_plugin_t *plugin)
{
    ...
    if (0 == rv) {
        rv = mqtt_aggregate_start_timer(plugin);
    }
