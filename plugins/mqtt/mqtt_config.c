                      ...
                      mqtt_config_t *config)
{
    ...
    neu_json_elem_t auth_enable  = {
        .name       = "auth-enable",
        .t          = NEU_JSON_BOOL,
        .v.val_bool = false,
        .attribute  = NEU_JSON_ATTRIBUTE_OPTIONAL,
    };
    neu_json_elem_t keylink = {
        .name      = "keylink",
        .t         = NEU_JSON_STR,
        .v.val_str = strdup("abcef13t6222t"),
        .attribute = NEU_JSON_ATTRIBUTE_OPTIONAL,
    };
    neu_json_elem_t auth_secret = {
        .name      = "auth-secret",
        .t         = NEU_JSON_STR,
        .v.val_str = strdup(""),
        .attribute = NEU_JSON_ATTRIBUTE_OPTIONAL,
    };
    neu_json_elem_t aggregate_mode = {
        .name      = "aggregate-mode",
        .t         = NEU_JSON_INT,
        .v.val_int = 0,
        .attribute = NEU_JSON_ATTRIBUTE_OPTIONAL,
    };
    neu_json_elem_t aggregate_window_ms = {
        .name      = "aggregate-window-ms",
        .t         = NEU_JSON_INT,
        .v.val_int = 60000,
        .attribute = NEU_JSON_ATTRIBUTE_OPTIONAL,
    };
    neu_json_elem_t sensor_timeout_ms = {
        .name      = "sensor-timeout-ms",
        .t         = NEU_JSON_INT,
        .v.val_int = 180000,
        .attribute = NEU_JSON_ATTRIBUTE_OPTIONAL,
    };
    neu_json_elem_t emit_partial = {
        .name       = "emit-partial",
        .t          = NEU_JSON_BOOL,
        .v.val_bool = true,
        .attribute  = NEU_JSON_ATTRIBUTE_OPTIONAL,
    };
    neu_json_elem_t aggregate_topic = {
        .name      = "aggregate-topic",
        .t         = NEU_JSON_STR,
        .v.val_str = strdup("neuron/${node}/aggregate"),
        .attribute = NEU_JSON_ATTRIBUTE_OPTIONAL,
    };
    neu_json_elem_t aggregate_sensor = {
    ...
    ret = neu_parse_param(setting, NULL, 1, &auth_enable);
    if (0 != ret) {
        plog_notice(plugin, "setting auth-enable failed");
        if (0 == strcmp(neu_plugin_module.module_name, "MQTT Auth")) {
            auth_enable.v.val_bool = true;
        }
    }

    ret = neu_parse_param(setting, NULL, 1, &keylink);
    if (0 != ret) {
        plog_notice(plugin, "setting keylink failed");
    }

    ret = neu_parse_param(setting, NULL, 1, &auth_secret);
    if (0 != ret) {
        plog_notice(plugin, "setting auth-secret failed");
    }

    ret = neu_parse_param(setting, NULL, 1, &aggregate_mode);
    if (0 != ret) {
        plog_notice(plugin, "setting aggregate-mode failed");
        if (0 == strcmp(neu_plugin_module.module_name, "MQTT Aggregate")) {
            aggregate_mode.v.val_int = MQTT_AGGREGATE_RAW_BATCH;
        }
    }

    ret = neu_parse_param(setting, NULL, 1, &aggregate_window_ms);
    if (0 != ret) {
        plog_notice(plugin, "setting aggregate-window-ms failed");
    }

    ret = neu_parse_param(setting, NULL, 1, &sensor_timeout_ms);
    if (0 != ret) {
        plog_notice(plugin, "setting sensor-timeout-ms failed");
    }

    ret = neu_parse_param(setting, NULL, 1, &emit_partial);
    if (0 != ret) {
        plog_notice(plugin, "setting emit-partial failed");
    }

    ret = neu_parse_param(setting, NULL, 1, &aggregate_topic);
    if (0 != ret) {
        plog_notice(plugin, "setting aggregate-topic failed");
    }

    ret = neu_parse_param(setting, NULL, 1, &aggregate_sensor);
    if (0 != ret) {
        plog_notice(plugin, "setting aggregate-sensor failed");
    }

    if (auth_enable.v.val_bool && strlen(keylink.v.val_str) == 0) {
        plog_error(plugin, "setting empty keylink");
        goto error;
    }
    if (aggregate_mode.v.val_int < MQTT_AGGREGATE_DISABLED ||
        aggregate_mode.v.val_int > MQTT_AGGREGATE_AGG_MULTI_SENSOR) {
        plog_error(plugin, "setting invalid aggregate-mode");
        goto error;
    }
    if (aggregate_mode.v.val_int != MQTT_AGGREGATE_DISABLED &&
        aggregate_window_ms.v.val_int <= 0) {
        plog_error(plugin, "setting invalid aggregate-window-ms");
        goto error;
    }
    ...
    config->auth_enable         = auth_enable.v.val_bool;
    config->keylink             = keylink.v.val_str;
    config->auth_secret         = auth_secret.v.val_str;
    config->aggregate_mode      = aggregate_mode.v.val_int;
    config->aggregate_window_ms = aggregate_window_ms.v.val_int;
    config->sensor_timeout_ms   = sensor_timeout_ms.v.val_int;
    config->emit_partial        = emit_partial.v.val_bool;
    config->aggregate_topic     = aggregate_topic.v.val_str;
    config->aggregate_sensor    = aggregate_sensor.v.val_str;
    ...
    plog_notice(plugin, "config auth-enable     : %d", config->auth_enable);
    if (config->auth_enable) {
        plog_notice(plugin, "config keylink         : %s", config->keylink);
        if (config->auth_secret && strlen(config->auth_secret) > 0) {
            plog_notice(plugin, "config auth-secret     : %s", placeholder);
        }
    }
    plog_notice(plugin, "config aggregate-mode  : %d",
                config->aggregate_mode);
    if (config->aggregate_mode != MQTT_AGGREGATE_DISABLED) {
        plog_notice(plugin, "config aggregate-window-ms : %" PRIu32,
                    config->aggregate_window_ms);
        plog_notice(plugin, "config sensor-timeout-ms   : %" PRIu32,
                    config->sensor_timeout_ms);
        plog_notice(plugin, "config emit-partial        : %d",
                    config->emit_partial);
        plog_notice(plugin, "config aggregate-topic     : %s",
                    config->aggregate_topic);
        if (config->aggregate_sensor && strlen(config->aggregate_sensor) > 0) {
            plog_notice(plugin, "config aggregate-sensor    : %s",
                        config->aggregate_sensor);
        }
    }
    if (config->ca) {
...
error:
    ...
    free(keylink.v.val_str);
    free(auth_secret.v.val_str);
    free(aggregate_topic.v.val_str);
    free(aggregate_sensor.v.val_str);
...
void mqtt_config_fini(mqtt_config_t *config)
{
    ...
    free(config->keypass);
    free(config->keylink);
    free(config->auth_secret);
    free(config->aggregate_topic);
    free(config->aggregate_sensor);
